...
export default function Home({ listings }: HomeProps) {
  console.log('props are ', listings);
          ...
          <div className="w-desktop h-[438px] flex items-center justify-between py-7">
            <div className="w-1/2 flex flex-col gap-4  py-7 pr-9 text-main-blue">
              ...
              <p className="text-xl font-light ">
                Ākina’s Impact Directory is where you can find all of our
                certified Impact Suppliers. These organisations deliver a range
                of positive social, environmental, and culturaloutcomes through
        ...
        <section className="w-screen flex justify-center pt-[88px]">
          <div className="flex justify-between w-desktop">
            <div className="flex flex-col gap-8">
              {listings
                .filter((d, index) => index < firstCol)
                .map((d) => {
                  return (
                    <Fragment key={d.id}>
                      <DirectoryCard data={d} />
                    </Fragment>
                  );
                })}
            </div>
            <div className="flex flex-col gap-8">
              {listings
                .filter(
                  (d, index) =>
                    index >= firstCol && index < firstCol + secondCol,
                )
                .map((d) => {
                  return (
                    <Fragment key={d.name}>
            ...
            <div className="flex flex-col gap-8">
              {listings
                .filter((d, index) => index >= firstCol + secondCol)
                .map((d) => {
                  return (
                    <Fragment key={d.name}>
                      <DirectoryCard data={d} />
